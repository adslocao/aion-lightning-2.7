...
public class MySQL5PlayerQuestListDAO extends PlayerQuestListDAO {
	...
	public static final String SELECT_QUERY = "SELECT `quest_id`, `status`, `quest_vars`, `quest_vars2`, `complete_count`, `next_repeat_time`, `reward`, `complete_time` FROM `player_quests` WHERE `player_id`=?";
	public static final String UPDATE_QUERY = "UPDATE `player_quests` SET `status`=?, `quest_vars`=?, `quest_vars2`=?, `complete_count`=?, `next_repeat_time`=?, `reward`=?, `complete_time`=? WHERE `player_id`=? AND `quest_id`=?";
	public static final String DELETE_QUERY = "DELETE FROM `player_quests` WHERE `player_id`=? AND `quest_id`=?";
	public static final String INSERT_QUERY = "INSERT INTO `player_quests` (`player_id`, `quest_id`, `status`, `quest_vars`, `quest_vars2`, `complete_count`, `next_repeat_time`, `reward`, `complete_time`) VALUES (?,?,?,?,?,?,?,?,?)";
	...
	public QuestStateList load(final Player player) {
		...
		try {
			...
			while (rset.next()) {
				...
				int questVars2 = rset.getInt("quest_vars2");
				...
				QuestState questState = new QuestState(questId, status, questVars, questVars2, completeCount, nextRepeatTime, reward, completeTime);
