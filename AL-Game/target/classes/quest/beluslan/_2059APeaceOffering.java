...
public class _2059APeaceOffering extends QuestHandler {
	...
	private final static int[] npc_ids = { 204787, 204795, 204796, 204702, 204806 };
	...
	public void register() {
		...
		qe.registerQuestNpc(204806).addOnLostTargetEvent(questId);
		qe.registerQuestNpc(204806).addOnReachTargetEvent(questId);
	...
	public boolean onNpcReachTargetEvent(QuestEnv env) {
		...
		return defaultFollowEndEvent(env, 3, 4, false); // 4
	}

	@Override
	public boolean onNpcLostTargetEvent(QuestEnv env) {
		return defaultFollowEndEvent(env, 3, 2, false); // 2
	}
	...
	public boolean onDialogEvent(QuestEnv env) {
		...
		else if (targetId == 204806) {
			if (var == 3) {
				final Npc npc = (Npc) env.getVisibleObject();
				npc.getAi2().onCreatureEvent(AIEventType.FOLLOW_ME, player);
				player.getController().addTask(TaskId.QUEST_FOLLOW, 
						QuestTasks.newFollowingToTargetCheckTask(env, 204813)); //204813 => Kalitan || 204813 => df3_hiddennpc_q2059
				return true;
			}
		}
		return false;
