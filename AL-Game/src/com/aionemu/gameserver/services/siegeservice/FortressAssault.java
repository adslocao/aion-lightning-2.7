		...
		dredgionTask =	ThreadPoolManager.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					spawnTask = ThreadPoolManager.getInstance().schedule(new Runnable() {
						...
						}, (long)(Rnd.get(240, 300)*1000));
					
				}
				
		},  (long) (delay * 1000 * SiegeConfig.BALAUR_ASSAULT_SPEED_RATE));
	}
	@Override
	...
	private void spawnAttackers() {
		...
		for( int i = 0; amount > i; i++) {
			...
			if ((i > Math.round(amount / 3)) && !spawnLocations.isEmpty()) {
				...
				spawn = SpawnEngine.addNewSiegeSpawn(worldId, templateId, locationId, 
						SiegeRace.BALAUR, SiegeModType.ASSAULT, coords[0] , coords[1], coords[2], heading);
				...
				spawn = SpawnEngine.addNewSiegeSpawn(worldId, templateId, locationId, 
					SiegeRace.BALAUR, SiegeModType.ASSAULT, x + x1 , y + y1, z, heading);
				attaker = (Npc) SpawnEngine.spawnObject(spawn, 1);
			}
			spawnedNpc.add(attaker);
		}		
